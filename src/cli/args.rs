...
#[clap(
    ...
    long_about = "vox-hash is a powerful command-line tool for hashing strings with SHA1 or MD5 and performing brute-force decryption of hashes. It supports single and bulk operations, customizable charsets, wordlists, patterns, and rainbow tables. Use --noverbose to reduce output. Ideal for security testing and hash analysis.\n\nCHARSETS:\n  - alphanumeric: a-z, 0-9\n  - lowercase: a-z\n  - uppercase: A-Z\n  - digits: 0-9\n  - custom: user-defined characters via --charset\n\nPATTERNS:\n  Patterns use the format [charset]{length}. \n  - [a-z]{4} matches any 4-letter lowercase string.\n  - [0-9]{6} matches any 6-digit string.\n  - [a-zA-Z0-9]{3} matches 3 alphanumeric characters.\n  - Character ranges like a-z, A-Z, 0-9 are supported: [a-c1-3]{5}.",
    after_help = "EXAMPLES:\n  vox-hash enc --algo sha1 --str 'test'              # Hash 'test' with SHA1\n  vox-hash dec --key 5baa61e4... --auto --wordlist words.txt  # Decrypt hash using wordlist\n  vox-hash dec --key 098f6bcd... --pattern '[a-z]{4}'         # Decrypt using pattern\n  vox-hash bulk-enc --algo md5 --input strings.txt            # Bulk hash strings from file\n  vox-hash bulk-dec --input hashes.txt --auto --only-success  # Bulk decrypt hashes, showing only hits\n  vox-hash generate-table --output table.json --max-len 3 --algo md5  # Generate a rainbow table"
)]
pub struct Cli {
    #[clap(subcommand)]
