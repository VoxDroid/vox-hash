...
fn test_enc_sha1() {
    ...
    cmd.assert()
       .success()
       .stdout(predicate::str::contains("a94a8fe5ccb19ba61c4c0873d391e987982fbbd3"));
}
...
fn test_dec_wordlist() {
    ...
    let mut cmd = Command::cargo_bin("vox-hash").unwrap();
    cmd.arg("dec")
       .arg("--key").arg("5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8") // "password"
       .arg("--wordlist").arg(words_path)
       .arg("--max-len").arg("10")
       .arg("--noverbose");
    cmd.assert()
       .success()
...
fn test_dec_brute_force() {
    let mut cmd = Command::cargo_bin("vox-hash").unwrap();
    cmd.arg("dec")
       .arg("--key").arg("098f6bcd4621d373cade4e832627b4f6") // "test"
       .arg("--algo").arg("md5")
       .arg("--min-len").arg("4")
       .arg("--max-len").arg("4")
       .arg("--charset-type").arg("lowercase")
       .arg("--noverbose");
    cmd.assert()
       .success()
...
fn test_generate_table_and_dec() {
    ...
    gen_cmd.arg("generate-table")
           .arg("--output").arg(table_path)
           .arg("--min-len").arg("1")
           .arg("--max-len").arg("3")
           .arg("--algo").arg("md5")
           .arg("--noverbose");
    ...
    dec_cmd.arg("dec")
           .arg("--key").arg("0cc175b9c0f1b6a831c399e269772661") // "a"
           .arg("--algo").arg("md5")
           .arg("--rainbow-table").arg(table_path)
           .arg("--noverbose");
    dec_cmd.assert()
           .success()
